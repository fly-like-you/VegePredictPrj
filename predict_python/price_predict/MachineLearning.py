...
project_path = "C:\\Users\\Parkjunho\\IdeaProjects\\VegePredictPrj\\predict_python\\price_predict\\vegetable_price\\"
def run():
    # 크롤링한 데이터 가져오기
    original_path = project_path + 'original_vegetable_csv\\'
    test_path = project_path + 'test.csv'
    train_path = project_path + 'train.csv'
    ...
    for day in days:
        for feature in features:
            ...
            if day == 1:
              day1.append(int(my_nong1.get_price()))
            if day == 2:
              day2.append(int(my_nong1.get_price()))
            if day == 3:
              day3.append(int(my_nong1.get_price()))
            if day == 4:
              day4.append(int(my_nong1.get_price()))
            if day == 5:
              day5.append(int(my_nong1.get_price()))
            if day == 6:
              day6.append(int(my_nong1.get_price()))
            if day == 7:
              day7.append(int(my_nong1.get_price()))
    ...
    models = dict(zip(["청양", "시금치(일반)", "토마토(일반)",
                       "파프리카(일반)", "깻잎(일반)", "미나리(일반)",
                       "양파(일반)", "풋고추(전체)"],
                      [c, s, t, p, g, m, y, u]))
    ...
    output_path = "C:\\Users\\Parkjunho\\IdeaProjects\\VegePredictPrj\\predict_python\\data\\predict\\"
